...
def home():
    ...
    followers = (
        Follower.query.with_entities(Follower.follower_id)
...
def edit_profile():
    if request.method == "POST":
        ...
        f.save(
            "app/static/profile/" + secure_filename(str((current_user.id))) + ".jpeg"
        )
...
def followers():
    users = User.query.all()
    followers = (
        Follower.query.with_entities(Follower.user_id)
...
def create_post():
    if request.method == "POST":
        ...
        f.save(
            ...
            + str(current_user.id)
            + "."
            + secure_filename(str((localtime)))
            + ".jpeg"
        )
        caption = request.form.get("caption")
        path = (
            "/static/posts/" + str(current_user.id) + "." + str((localtime) + ".jpeg")
        )
        path = path.replace(" ", "_")
        path = path.replace(":", "")
...
def unfollow(user_id):
    user = Follower.query.filter_by(
        user_id=current_user.id, follower_id=user_id
    ).first()
